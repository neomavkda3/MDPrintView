#!/usr/bin/env bash
# Smoke-test MDPrintView: open a sample markdown file, verify the app stays alive,
...
if [ -z "$APP_PATH" ]; then
    echo "FAIL: MDPrintView.app not built. Run 'xcodebuild build' first." >&2
    exit 1
fi

SAMPLE=$(mktemp -t MDPrintView-smoke).md
trap "rm -f \"$SAMPLE\"; osascript -e 'tell application \"MDPrintView\" to quit' 2>/dev/null; pkill -x MDPrintView 2>/dev/null || true" EXIT
...
BASELINE_CRASH=$(ls -t ~/Library/Logs/DiagnosticReports/MDPrintView-*.ips 2>/dev/null | head -1 || true)
...
LATEST_CRASH=$(ls -t ~/Library/Logs/DiagnosticReports/MDPrintView-*.ips 2>/dev/null | head -1 || true)
